#include <stdio.h>
#include <stdlib.h>
/*a 127 ex 16 pg jogo da velha ao final imprima o resultado do jogo e pergunte se deseja
jogar vovamente.
*/
int main()
{
    int l, c, linha, coluna, jogador , ganhou , jogadas, opc ;
    // estrutua de dados?
    char jogo[3][3];

    do{
        jogador =1;
        ganhou = 0;
        jogadas = 0;

        // comoo iniciar nossa estrutura de dados?

        for(l=0;l<3;l++){
            for(c=0;c<3;c++){
                jogo[l][c] = ' ';
            }
        }

        do{
        // imprimir jogo
            printf("\n\n\t 0   1   2\n\n");
            for(l = 0; l<3; l++){
                for(c=0; c<3; c++){
                    if(c==0)
                        printf("\t");
                    printf(" %c ", jogo[l][c]);
                    if(c<2)
                        printf("|");
                    if(c == 2)
                        printf("  %d", l);

                }
                if(l<2)
                    printf("\n\t-----------");
                printf("\n");
            }

            // ler coordenadas
            do{
                printf("JOGADOR %d: digite a linha e a coluna que deseja jogar: ", jogador);
                scanf("%d%d", &linha, &coluna);
            }while(linha < 0 || linha >2|| coluna < 0 || coluna >2 || jogo[linha][coluna] != ' ');


            // salvar coordenadas
            if(jogador==1) {
                jogo[linha][coluna] ='0';
                jogador++;
            }
            else{
                jogo[linha][coluna] = 'x';
                jogador =1;
            }
            jogadas++;

            //alguem ganhou?
            //alguem ganhou por linha
            if(jogo[0][0] == 'x' && jogo[0][1] == 'x' && jogo[0][2]=='x' ||
               jogo[1][0] == 'x' && jogo[1][1] == 'x' && jogo[1][2]=='x' ||
               jogo[2][0] == 'x' && jogo[2][1] == 'x' && jogo[2][2]=='x' ){
                printf("\nparabens! O jogador 1 venceu! por linha\n");
                ganhou = 1;
               }

            if(jogo[0][0] == 'x' && jogo[0][1] == 'x' && jogo[0][2]=='x' ||
               jogo[1][0] == 'x' && jogo[1][1] == 'x' && jogo[1][2]=='x' ||
               jogo[2][0] == 'x' && jogo[2][1] == 'x' && jogo[2][2]=='x' ){
                printf("\nparabens! O jogador 2 venceu! por linha\n");
                ganhou = 1;
               }

            //alguem ganhou por coluna
            if(jogo[0][0] == '0' && jogo[1][0] == '0' && jogo[2][0]=='0' ||
               jogo[0][1] == '0' && jogo[1][1] == '0' && jogo[2][1]=='0' ||
               jogo[0][2] == '0' && jogo[1][2] == '0' && jogo[2][2]=='0' ){
                printf("\nparabens! O jogador 1 venceu! por coluna\n");
                ganhou =1;
        }

            if(jogo[0][0] == 'x' && jogo[1][0] == 'x' && jogo[2][0]=='x' ||
               jogo[0][1] == 'x' && jogo[1][1] == 'x' && jogo[2][1]=='x' ||
               jogo[0][2] == 'x' && jogo[1][2] == 'x' && jogo[2][2]=='x' ){
                printf("\nparabens! O jogador 2 venceu! por coluna\n");
                ganhou = 1;
                }


            //alguem ganhou na diagonal principal
            if(jogo[0][0] == '0' && jogo[1][1] == '0' && jogo[2][2] == '0' ){
                printf("\nparabens! O jogador 1 venceu! por diagonal principal\n");
                 ganhou = 1;
               }

            if(jogo[0][0] == 'x' && jogo[1][1] == 'x' && jogo[2][2] == 'x' ){
                printf("\nparabens! O jogador 2 venceu! por diagonal principal\n");
                 ganhou = 1;
               }

            // alguem ganhou   na diagonal decundaria

            if(jogo[0][2] == '0' && jogo[1][1]== '0' && jogo[2][0] == '0'){
                printf("\nparabens! O jogador 1 venceu! via diagonal secundaria\n");
                ganhou = 1;
               }

            if(jogo[0][2] == 'x' && jogo[1][1]== 'x' && jogo[2][0] == 'x'){
                printf("\nparabens! O jogador 2 venceu! via diagonal secundaria\n");
                ganhou = 1;
               }

        }while(ganhou==0 && jogadas <9);
        if(ganhou == 0)
            printf("\no jogo finalizou sem ganhador!\n");


        printf("\n digite 1 para jogar novamente: \n");
        scanf("%d", &opc);

    }while(opc == 1);

    return 0;
}
